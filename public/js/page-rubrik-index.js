var table_artikel = $('#tabel_rubrik').DataTable({
    processing: true,
    serverSide: false,
    ...
    columns   : [
        ...
        {data : "id", render: function(data) {
            ...
            return '<div id="buttons" class="d-grid gap-2 d-md-block"><a href="'+url_edit+'" class="btn btn-info btn-block btn-sm" type="button"><i class="fas fa-pencil-alt"></i>&nbsp; Edit</a><button class="delete-artikel btn btn-danger my-2 btn-block btn-sm" onclick="destroy('+data+');" type="button"><i class="fas fa-trash-alt"></i>&nbsp; Delete</button><button class="btn btn-info my-2 btn-block btn-sm" type="button"><i class="far fa-eye"></i>&nbsp; View</button></div>';
        }}
    ],
});
...
function destroy(id){
    ...
    var cnfrm = confirm("Apakah rubrik ini akan di hapus?");
    if(cnfrm === true) {
        $.ajax({
            url: url_delete,
            method: "DELETE",
